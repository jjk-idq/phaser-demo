...
export function createAnimations(scene: Scene) {
    ...
    sexes.forEach(sex => {
        gears.forEach(gear => {
            weapons.forEach(weapon => {
                anims.create({ key: `${sex}-${gear}-${weapon}-idle-up`, frames: anims.generateFrameNumbers(`${sex}-${gear}-${weapon}-walk`, { start: 0, end: 1 }), frameRate: 4, repeat: -1 });
                anims.create({ key: `${sex}-${gear}-${weapon}-idle-left`, frames: anims.generateFrameNumbers(`${sex}-${gear}-${weapon}-walk`, { start: 9, end: 10 }), frameRate: 4, repeat: -1 });
                anims.create({ key: `${sex}-${gear}-${weapon}-idle-down`, frames: anims.generateFrameNumbers(`${sex}-${gear}-${weapon}-walk`, { start: 18, end: 19 }), frameRate: 4, repeat: -1 });
                anims.create({ key: `${sex}-${gear}-${weapon}-idle-right`, frames: anims.generateFrameNumbers(`${sex}-${gear}-${weapon}-walk`, { start: 27, end: 28 }), frameRate: 4, repeat: -1 });    
            });
        });        
    });
